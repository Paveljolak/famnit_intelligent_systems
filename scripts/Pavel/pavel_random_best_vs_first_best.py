# Random best vs First best

# Summary: 
# First best destroys random best 5/5. 
...
def minimax(model, depth, alpha, beta, maximizing_player):
    ...
    if maximizing_player:
        ...
        for move in model.legal_moves(player=1):
            ...
            if beta <= alpha:
                break
        # Player 1 picks the best move
        #  return max_eval, random.choice(best_moves) if best_moves else None
        return max_eval, best_moves[0] if best_moves else None
    else:
        min_eval = float('inf')
        best_moves = []
        ...
        # return min_eval, best_moves[0] if best_moves else None
...
def evaluate2(model, player):
    ...
    if(phase == 'placing'):
        ...
        evaluation = (
            ...
            2 * blocked_pieces(state, player) +
            8 * model.count_pieces(player) +
            1000 * win(model, player)
        )
    elif phase == "flying":
        opponent = 2 if player == 1 else 1
        num_pieces = model.count_pieces(player)
        num_opponent_pieces = model.count_pieces(opponent)
        morrises = count_morrises(state, player)
        opp_morrises = count_morrises(state, opponent)
        two_config = two_piece_configuration(state, player)
        opp_two_config = two_piece_configuration(state, opponent)
        won = win(model, player)        

        evaluation = (
        60 * (morrises - opp_morrises) +
        40 * two_config -
        20 * opp_two_config +
        150 * (num_pieces - num_opponent_pieces) +
        1500 * won
        )
